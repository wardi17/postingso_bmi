        ...
        <div class="card mt-4">
          ...
          <div class="col-sm-12 mt-2 mb-2">
              <div class="text-center">
                <button type="btn"  id="Adddata" class="btn btn-primary">Submit</button>
  ...
  $(document).ready(function(){
    ...
    $("#Adddata").on("click",function(e){
        ...
        let bmno = $("#bmano").val();
         ...
         ajaxPosting(dataposting);
    })
});


function ajaxPosting(dataposting){
  $.ajax({
              url:"<?=base_url?>/postingso/postingok",
                method:"POST",
                data:dataposting,
                dataType: "json",
                beforeSend: function(){
...
function TemptPosting(){
 ...
 // let total = $("#total").val();
  ...
  //     'price': replacekoma($(this).find('td').eq(3).text()),
  //     'qty':  $(this).find('td').eq(4).text(),
  //     'disc':  $(this).find('td').eq(5).text(),
  //     'hargax':replacekoma($(this).find('td').eq(6).text()),
  //     'isi':replacekoma($(this).find('td').eq(7).text()),
  //     'amount':replacekoma($(this).find('td').eq(8).text()),
...
function goBack(){
   let cabang = $("#CompanyID").val();
  
  const  halamanSebelumnya ="<?=base_url3?>";
